...
function gateAvg(score: number | null): string {
  ...
  // scores are /10, averages /4 in template (5 questions each scoring 1-4 = 20 max)
  // gate average = score / 2.5  (10 raw maps to 4.0 avg scale)
...
function arcColor(pct: number): string {
  if (pct < 37) return "#a63232"; // red
  if (pct < 62) return "#b07a14"; // orange
  return "#2D7A4F"; // green
}
...
function getStr(page: any, prop: string, type: string): string {
  // We just use raw page properties since we need Lead fields not in parseScore
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function POST(req: NextRequest) {
  ...
  // 3. Fetch Score from Notion
  const scorePage = await notion.pages.retrieve({ page_id: score_id });
  if (!isFullPage(scorePage)) {
    return NextResponse.json({ error: "Score page not found" }, { status: 404 });
  }
  const score = parseScore(scorePage);

  // 4. Derive gate metrics
  const rawScore = score.rawScore ?? 0;
  const scorePct = score.scorePct ?? Math.round((rawScore / 50) * 100);
  // Weighted: gate weights 1.5, 1.0, 1.0, 1.5, 1.25 → max 125
...
{
  "executive_summary": "2-3 sentences describing the overall structural condition of this specific business. Reference the actual scores and classification. Be direct and specific to this company — never generic.",
  "hero_headline": "A single declarative sentence (under 12 words) that captures the core structural problem. Do not use the word 'the'. No period at end.",
  "fragility_intro": "One sentence (under 30 words) describing how the top fragility vectors compound each other for this specific business.",
  "fragility_1_title": "Short title (2-4 words) for the most critical fragility vector",
  "fragility_1_body": "2-3 sentences describing this fragility vector with specifics relevant to a ${industry} business scoring ${g1}/10 on Authority.",
  ...
  "fragility_2_body": "2-3 sentences for this vector, relevant to the specific score data.",
  ...
  "gate_1_analysis": "2 sentences analyzing Gate 1 (Authority/Decision Rights) based on the actual score ${g1}/10. Reference what this means for SBA underwriting.",
  "gate_2_analysis": "2 sentences on Gate 2 (Process Independence) based on score ${g2}/10.",
  "gate_3_analysis": "2 sentences on Gate 3 (Commercial Discipline) based on score ${g3}/10.",
  "gate_4_analysis": "2 sentences on Gate 4 (Revenue Quality) based on score ${g4}/10.",
  "gate_5_analysis": "2 sentences on Gate 5 (Financial Integrity) based on score ${g5}/10.",
  "path_why_now": "One bold statement (under 10 words) for why Tier II is urgent for this specific business.",
  "path_why_now_body": "2-3 sentences explaining the structural urgency specific to their scores.",
  "path_inaction": "One bold statement (under 10 words) about the cost of not acting.",
  "path_inaction_body": "2-3 sentences on what happens if no structural correction occurs.",
  "move_1_title": "Action title for highest-priority structural move (imperative, under 8 words)",
  "move_1_body": "2-3 sentences describing this specific action for this business and industry.",
  "move_1_tag": "G[N] · [criterion name]",
  "move_1_impact": "One sentence on structural impact of this move.",
  "move_2_title": "Second action title",
  "move_2_body": "2-3 sentences.",
  "move_2_tag": "G[N] · [criterion name]",
  "move_2_impact": "One sentence on structural impact.",
  "move_3_title": "Third action title",
  "move_3_body": "2-3 sentences.",
  "move_3_tag": "G[N] · [criterion name]",
  "move_3_impact": "One sentence on structural impact.",
  "move_4_title": "Fourth action title",
  "move_4_body": "2-3 sentences.",
  "move_4_tag": "G[N] · [criterion name]",
  "move_4_impact": "One sentence on structural impact.",
  "move_5_title": "Fifth action title",
  "move_5_body": "2-3 sentences.",
  "move_5_tag": "G[N] · [criterion name]",
  "move_5_impact": "One sentence on structural impact.",
  "rescore_headline": "One sentence about what Tier II re-scoring could achieve for this business (under 15 words).",
  "rescore_summary": "2-3 sentences on how structural correction expands the buyer pool and multiple for this specific business.",
  "compression_headline": "One sentence (under 12 words) describing the current SDE multiple situation.",
  "compression_body": "2 sentences on how the gate scores compress the multiple for this business."
}

Respond with ONLY valid JSON. No markdown, no commentary, no code blocks. Just the JSON object.`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Claude returned invalid JSON", raw: rawJson.slice(0, 500) },
      { status: 500 }
    );
  }
  ...
  // 8. Store in Vercel Blob
  const slug = crypto.randomBytes(6).toString("hex"); // 12-char random slug
  const blobPath = `reports/${slug}/index.html`;
  ...
  // 9. Write URL back to Notion score record
  await notion.pages.update({
    page_id: score_id,
    properties: {
      "Report Draft URL": { url: reportUrl },
